...
export default class BambuDriver extends OAuth2Device {
  ...
  async onState(state) {
    ...
    const hasCameraStreamingWithWebRTCProxy = typeof this.homey.hasFeature === 'function'
      && this.homey.hasFeature('camera-streaming')
      && this.homey.platform === 'local'
      && semver.satisfies(this.homey.version.split('-')[0], '>=12.12.0');

    // Register Video
    if (typeof state?.print?.ipcam?.rtsp_url === 'string' && !this.video && hasCameraStreamingWithWebRTCProxy) {
      this.video = Promise.resolve().then(async () => {
        ...
        .then(() => this.log('Registered Camera'))
        .catch(err => this.error(`Error Registering Camera: ${err.message}`));
    }
  }
